...
class Trainer(object):
    ...
    def train(self, validate=False):
        ...
        import pickle
        with open("../yolov5/yolov5s.pkl", "rb") as f:
            torch_params = pickle.load(f)
        for name, param in model.named_parameters():
            if name[name.rindex('.')+1:] == "_mean":
                name = name.replace("_mean", "running_mean")
            elif name[name.rindex('.')+1:] == "_variance":
                name = name.replace("_variance", "running_var")
            startwith = name[:name.index('.')]
            if startwith == "backbone":
                torch_name = name[name.index('.')+1:]
                param.set_value(torch_params[torch_name].data.cpu().numpy())
            elif startwith == "neck":
                s = name.split('.')
                torch_name = "model." + str(int(s[2]) + 10) + '.' + '.'.join(s[3:])
                param.set_value(torch_params[torch_name].data.cpu().numpy())
            else: # yolo_head
                s = name.split('.')
                torch_name = "model.24.m." + '.'.join(s[2:])
                param.set_value(torch_params[torch_name].data.cpu().numpy())


        
        # for ppair, tpair in zip(model.named_parameters(), torch_params.items()):
        #    print(ppair[0])
        #    print(tpair[0])
        #    print(np.max(abs(ppair[1].detach().cpu().numpy()-tpair[1].detach().cpu().numpy())))
            #print(ppair)
            #print(tpair)
        ...
        for epoch_id in range(self.start_epoch, self.cfg.epoch):
            ...
            for step_id, data in enumerate(self.loader):
                ...
                # np.save("./input_paddle.npy", data['image'].cpu().numpy())
                ...
                if self.cfg.get('fp16', False):
                    ...
                    outputs = model(data)
                    #y = outputs.sum()
                    y = sum([feat.sum() for feat in outputs])
                    y.backward()                    
                    #loss = outputs['loss']
                    # model backward
                    #loss.backward()
                    self.optimizer.step()
                    #np.save("backbone_output0.npy", outputs[0].detach().cpu().numpy())
                    #np.save("backbone_output1.npy", outputs[1].detach().cpu().numpy())
                    #np.save("backbone_output2.npy", outputs[2].detach().cpu().numpy())
                    np.save("img_grad_epoch{}_head.npy".format(epoch_id), data['image'].grad.detach().cpu().numpy())
