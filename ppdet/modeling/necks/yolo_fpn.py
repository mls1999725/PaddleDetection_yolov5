...
class YOLOv5FPN(nn.Layer):
    ...
    def __init__(self,
        ...
        for m in self.model:
            if m.i in feature_maps:
                self._out_channels.append(m.ch)
    ...
    def forward(self, blocks):
        ...
        for i, m in enumerate(self.model):
            ...
            #        np.save("neck_{}.npy".format(name), params.detach().cpu().numpy())
            ...
            if m.i in self.save:
                blocks.append(x)
            # if i == 13:
            #    np.save("neck_23_output.npy", x.detach().cpu().numpy())
            if m.i in self.feature_maps:
                # np.save("neck_{}_output.npy".format(m.i), x.detach().cpu().numpy())
                feats.append(x)
        return feats
