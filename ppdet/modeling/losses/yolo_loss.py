...
class YOLOv5Loss(nn.Layer):
    ...
    def forward(self, inputs, targets, anchors):
        #import numpy
        #numpy.save("output0.npy", inputs[0].detach().cpu().numpy())
        #numpy.save("output1.npy", inputs[1].detach().cpu().numpy())
        #numpy.save("output2.npy", inputs[2].detach().cpu().numpy())
        np = len(inputs)
        gt_targets = [targets['target{}'.format(i)] for i in range(np)]
        yolo_losses = dict()
        for i, (x, t, anchor, downsample) in enumerate(zip(inputs, gt_targets, anchors,
                                            self.downsample)):
            na = len(anchor)
            b, c, h, w = x.shape
            ...
            x = x.reshape((b, self.nt_max, na, no, h, w)).transpose((0, 1, 2, 4, 5, 3))
            t = t.reshape((b, self.nt_max, na, no, h, w)).transpose((0, 1, 2, 4, 5, 3))
            #import numpy
            #numpy.save("loss_input_{}".format(i), x.cpu().numpy())
