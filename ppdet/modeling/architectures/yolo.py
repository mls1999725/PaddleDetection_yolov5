...
class YOLOv5(BaseArch):
    ...
    def _forward(self):
        body_feats = self.backbone(self.inputs)
        #np.save("backbone_paddle_4.npy", body_feats[0].cpu().numpy())
        #np.save("backbone_paddle_6.npy", body_feats[1].cpu().numpy())
        #np.save("backbone_paddle_9.npy", body_feats[2].cpu().numpy())
        neck_feats = self.neck(body_feats)
        #np.save("neck_output_17.npy", neck_feats[0].detach().cpu().numpy())
        #np.save("neck_output_20.npy", neck_feats[1].detach().cpu().numpy())
        #np.save("neck_output_23.npy", neck_feats[2].detach().cpu().numpy())
