...
class Conv(nn.Layer):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        ...
        self.bn = nn.BatchNorm2D(c2, momentum=0.97, epsilon=0.001)
...
class Focus(nn.Layer):
    ...
    def forward(self, x):  # x(b,c,w,h) -> y(b,4c,w/2,h/2)
        ...
        y2 = paddle.strided_slice(x, [2, 3], [1, 0], [h, w], [2, 2])
        y3 = paddle.strided_slice(x, [2, 3], [0, 1], [h, w], [2, 2])
        y4 = paddle.strided_slice(x, [2, 3], [1, 1], [h, w], [2, 2])
        y = paddle.concat([y1, y2, y3, y4], 1)
        #for name, param in self.conv.named_parameters():
        #    np.save("Focus_{}.npy".format(name), param.detach().cpu().numpy())
        out = self.conv(y)
        # np.save("Focus_output_paddle.npy", out.detach().cpu().numpy())
        return out
